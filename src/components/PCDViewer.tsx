...
export default function PCDViewer({ url }: PCDViewerProps) {
  ...
  const [pointOpacity, setPointOpacity] = useState<number>(0.15);
  ...
  useEffect(() => {
    iconSpeedRef.current = iconSpeed;
  }, [iconSpeed]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    loader.load(
      ...
      (points) => {
        ...
        glowMat.color = new THREE.Color(0x0033aa); // Darker blue to lower brightness
    ...
    const animate = () => {
      ...
      controls.enabled =
        !isAddingLabelRef.current && !selectedAnnotationRef.current;
      ...
      if (edlPassRef.current && composer.readBuffer) {
        edlPassRef.current.uniforms.tDepth.value =
      ...
      <div className="absolute bottom-4 left-4 bg-zinc-900/80 backdrop-blur-sm border border-zinc-800 p-4 rounded-lg flex flex-col gap-3 min-w-[240px] max-h-[80vh] overflow-y-auto z-30">
        ...
        <div className="border-t border-zinc-800 pt-3">
          <label className="text-xs text-zinc-300 flex justify-between mb-2">
            ...
            <span className="uppercase text-[10px] font-mono text-zinc-500">
              {backgroundColor}
            </span>
          </label>
